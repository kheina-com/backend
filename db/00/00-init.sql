...
CREATE PROCEDURE public.add_tags(
	sp_post_id character,
	sp_user_id bigint,
	sp_tags text []
) LANGUAGE plpgsql AS $$ BEGIN
INSERT INTO kheina.public.tag_post (tag_id, user_id, post_id) WITH sp_tag_ids AS (
		SELECT tag_to_id(unnest(sp_tags)) AS tag_id
	)
SELECT sp_tag_ids.tag_id,
	sp_user_id,
	sp_post_id
FROM public.sp_tag_ids
UNION
SELECT tag_inheritance.child,
	sp_user_id,
	sp_post_id
FROM public.sp_tag_ids
	INNER JOIN public.tag_inheritance ON tag_inheritance.parent = sp_tag_ids.tag_id ON CONFLICT DO NOTHING;
END;
$$;
--
...
CREATE FUNCTION public.context_to_id(_frame text) RETURNS smallint LANGUAGE sql AS $$
SELECT context_id
FROM public.context
WHERE frame = _frame
LIMIT 1;
...
CREATE FUNCTION public.create_new_post(sp_user_id bigint) RETURNS character LANGUAGE plpgsql AS $$
DECLARE sp_post_id CHAR(8);
BEGIN sp_post_id := (
...
INSERT INTO public.posts (uploader)
VALUES (sp_user_id)
...
CREATE PROCEDURE public.inherit_tag(
	sp_user_id bigint,
	sp_parent text,
	sp_child text,
	sp_deprecate boolean
) LANGUAGE plpgsql AS $$
DECLARE sp_parent_id INT;
DECLARE sp_child_id INT;
BEGIN sp_parent_id := public.tag_to_id(sp_parent);
sp_child_id := public.tag_to_id(sp_child);
INSERT INTO public.tag_inheritance (parent, child)
SELECT sp_parent_id,
	sp_child_id
UNION
SELECT sp_parent_id,
	tag_inheritance.child
FROM public.tag_inheritance
WHERE tag_inheritance.parent = sp_child_id ON CONFLICT DO NOTHING;
INSERT INTO public.tag_post (tag_id, post_id, user_id)
SELECT tag_inheritance.child,
	tag_post.post_id,
	tag_post.user_id
FROM public.tag_inheritance
	INNER JOIN public.tag_post ON tag_post.tag_id = tag_inheritance.parent
WHERE tag_inheritance.parent = sp_parent_id ON CONFLICT DO NOTHING;
...
CREATE FUNCTION public.media_file_type_to_id(_type text) RETURNS smallint LANGUAGE sql AS $$
SELECT media_type_id
FROM public.media_type
WHERE file_type = _type
LIMIT 1;
$$;
--
...
CREATE FUNCTION public.media_mime_type_to_id(_type text) RETURNS smallint LANGUAGE sql AS $$
SELECT media_type_id
FROM public.media_type
WHERE mime_type = _type
LIMIT 1;
$$;
--
...
CREATE FUNCTION public.new_post_id() RETURNS character LANGUAGE plpgsql AS $$
DECLARE fn_post_id CHAR(8);
BEGIN LOOP fn_post_id := replace(
...
WHEN (
	...
	WHERE word LIKE concat('%', LOWER(fn_post_id), '%')
) IS NULL
AND (
	SELECT 1
	FROM public.posts
	WHERE post_id = fn_post_id
) IS NULL;
END LOOP;
RETURN fn_post_id;
END;
...
CREATE FUNCTION public.privacy_to_id(_type text) RETURNS smallint LANGUAGE sql AS $$
SELECT privacy_id
FROM public.privacy
WHERE type = _type
LIMIT 1;
...
CREATE FUNCTION public.rating_to_id(_rating text) RETURNS smallint LANGUAGE sql AS $$
SELECT ratings.rating_id
FROM public.ratings
WHERE ratings.rating = LOWER(_rating)
LIMIT 1;
$$;
--
...
CREATE FUNCTION public.relation_to_id(_type text) RETURNS smallint LANGUAGE sql AS $$
SELECT relation_id
FROM public.relations
WHERE relation = _type
LIMIT 1;
$$;
--
...
CREATE PROCEDURE public.remove_tags(
	sp_post_id character,
	sp_user_id bigint,
	sp_tags text []
) LANGUAGE plpgsql AS $$
...
DELETE FROM public.tag_post USING kheina.public.tags
...
IF sp_user_id = any(sp_user_ids) THEN
DELETE FROM public.tag_post USING public.tags
WHERE tag_post.tag_id = tags.tag_id
	AND tag_post.post_id = sp_post_id
	AND tags.tag = any(sp_tags);
ELSE
...
CREATE FUNCTION public.tag_class_to_id(_class text) RETURNS smallint LANGUAGE sql AS $$
SELECT class_id
FROM public.tag_classes
WHERE class = _class
LIMIT 1;
$$;
--
...
CREATE FUNCTION public.tag_to_id(_tag text) RETURNS integer LANGUAGE plpgsql AS $$
DECLARE fn_tag_id INT;
BEGIN fn_tag_id := (
	SELECT tag_id
	FROM public.tags
	WHERE tag = _tag
...
CREATE FUNCTION public.user_to_id(_handle text) RETURNS bigint LANGUAGE sql AS $$
SELECT user_id
FROM public.users
...
CREATE PROCEDURE public.user_upload_file(
	sp_user_id bigint,
	INOUT sp_post_id character,
	sp_media_type text,
	sp_filename text
) LANGUAGE plpgsql AS $$ BEGIN IF sp_post_id IS NULL THEN sp_post_id := (
	SELECT kheina.public.create_new_post(sp_user_id)
);
END IF;
...
ALTER TABLE auth.bot_login
ALTER COLUMN bot_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME auth.bot_login_bot_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER TABLE auth.bot_type
ALTER COLUMN bot_type_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME auth.bot_type_bot_type_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER TABLE auth.token_keys
ALTER COLUMN key_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME auth.token_keys_key_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER COLUMN badge_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.badges_badge_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER COLUMN context_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.context_context_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER COLUMN media_type_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.media_type_media_type_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER COLUMN privacy_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.privacy_privacy_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.ratings_rating_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.relations_relation_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
ALTER TABLE public.tag_classes
ALTER COLUMN class_id
ADD GENERATED ALWAYS AS IDENTITY (
		SEQUENCE NAME public.tag_classes_class_id_seq START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
	);
--
...
CREATE UNIQUE INDEX bot_login_user_id_index ON auth.bot_login USING btree (user_id)
...
CREATE UNIQUE INDEX posts_uploader_unpublished_constraint ON public.posts USING btree (uploader, privacy_id)
WHERE (privacy_id = 4);
--
